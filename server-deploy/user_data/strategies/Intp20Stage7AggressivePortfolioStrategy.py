...
class Intp20Stage7AggressivePortfolioStrategy(IStrategy):
    ...
    btc_informative_pair = "BTC/USDT:USDT"
    ohlcv_columns = ("date", "open", "high", "low", "close", "volume")

    def informative_pairs(self):
        return [(self.btc_informative_pair, self.timeframe)]

    @classmethod
    def _has_ohlcv(cls, dataframe: DataFrame) -> bool:
        return (
            dataframe is not None
            and not dataframe.empty
            and set(cls.ohlcv_columns).issubset(dataframe.columns)
        )

    @staticmethod
    def _empty_btc_regime(include_roc_48: bool = False) -> DataFrame:
        columns = [
            "date",
            "btc_market_bull",
            "btc_market_bear",
        ...
        if include_roc_48:
            columns.append("btc_roc_48")
        return DataFrame(columns=columns)

    @staticmethod
    def _resample_5m(dataframe: DataFrame) -> DataFrame:
        if not Intp20Stage7AggressivePortfolioStrategy._has_ohlcv(dataframe):
            return DataFrame(columns=Intp20Stage7AggressivePortfolioStrategy.ohlcv_columns)
    ...
    def _btc_5m_regime(self) -> DataFrame:
        if not self.dp:
            return self._empty_btc_regime()
        btc = self.dp.get_pair_dataframe(pair=self.btc_informative_pair, timeframe=self.timeframe)
        if not self._has_ohlcv(btc):
            return self._empty_btc_regime()

        btc_5m = self._resample_5m(btc)
        if btc_5m.empty:
            return self._empty_btc_regime()

        btc_5m = self._add_5m_indicators(btc_5m)
        return btc_5m[
            [
                "date",
                "market_bull",
...
class Intp20Stage12XtzCoreRocFilterStrategy(Intp20Stage11CNoXtzMaoffStrategy):
    ...
    def _btc_5m_regime(self) -> DataFrame:
        if not self.dp:
            return self._empty_btc_regime(include_roc_48=True)
        btc = self.dp.get_pair_dataframe(pair=self.btc_informative_pair, timeframe=self.timeframe)
        if not self._has_ohlcv(btc):
            return self._empty_btc_regime(include_roc_48=True)
