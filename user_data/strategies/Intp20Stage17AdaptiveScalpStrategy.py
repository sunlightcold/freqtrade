...
class Intp20Stage17AdaptiveScalpStrategy(Intp20Stage15AggressiveScalpStrategy):
    ...
    def _is_protected_legacy_tag(cls, entry_tag: str | None) -> bool:
        if not entry_tag:
            return False
        return entry_tag.startswith(cls.protected_legacy_prefixes)
    ...
    def _legacy_stage_exit_reason(
        ...
        if trade_minutes >= 6 and current_profit >= 0.015:
            return f"{stage_label}_legacy_take_profit_exit"
        if trade_minutes >= 15 and current_profit >= 0.006:
            return f"{stage_label}_legacy_time_profit_exit"
        if trade_minutes >= 2 and current_profit <= -0.040:
            return f"{stage_label}_legacy_fast_loss_exit"
        if trade_minutes >= 8 and current_profit <= -0.025:
